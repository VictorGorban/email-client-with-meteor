...
Template.processMailModal.events({
    ...
    'click #processSend': async (e, t) => {
        ...
        mail.attachments.forEach(att => {
            att.content = bytesToBase64(att.content); // uint8Array to base64
        });
    ...
    'click #decipherProcessMail': async (e, t) => {
        ...
        Meteor.call('decipherAndVerifyEmail', mail, passphrase, keys.rsa.private, keys.dsa.public, (err, mail) => {
            if (err) {
                console.log(err);
                if (err.error !== 500) {
                    showError(err.error);
            ...
            console.log('in decipherAndVerifyEmail callback. Mail is');
            console.log(mail);
            if (mail) {
                Session.set('mailToProcess', mail);
            }
